...
const UPDATED = '2026-09-04';   // rewritten for the follow-up funnel, the audit tool and offline conversions
...
const privacy = shell({
  ...
  html: `
    ...
    <p>This site collects what you type into its forms, and a record of which pages were visited.
      We do not sell data and we do not share your contact details with advertisers. We do use
      Google's analytics and advertising tags, which set cookies — described below. If you tick the
      box on a form, you may get a small number of follow-up messages about that enquiry; one word
      back stops them for good.</p>
    ...
    <p>Visits are counted on our own server first, which needs no cookie and sends nothing
      anywhere. On top of that we use <strong>Google Analytics 4</strong> and <strong>Google Tag
      Manager</strong>, and <strong>Google Ads conversion tracking</strong> while we are running
      advertising. These load from Google, set cookies, and send Google information about your
      visit — the pages you saw, roughly when, and an identifier for your browser that is not your
      name. They are loaded only after the page has finished drawing, so they never slow it down
      for you, and the site works identically if your browser blocks them — most tracking
      protection does.</p>
    <p>Which of these are switched on is controlled from one place, and this section is updated
      when that changes.</p>
    ...
    <ul>
      ...
      <li><strong>Google</strong> — analytics and tag management, as described under Analytics and
        cookies above. Google acts on its own terms for that data; ours are that we send it nothing
        you typed into a form.</li>
